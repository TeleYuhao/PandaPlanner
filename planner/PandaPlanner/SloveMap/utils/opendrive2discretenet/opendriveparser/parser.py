    ...
    for sideTag, newSideLanes in sides.items():
        ...
        for lane in side.findall("lane"):
            ...
            if lane.find("roadMark") is not None:
                ...
                line_type = roadmark.get('type')
                line_color = roadmark.get('color')
                new_lane._roadMark = {'type':line_type,'color':line_color}
...
def parse_opendrive_header(opendrive, header):
    ...
    parsed_header.revMajor =  header.get("revMajor")  # _Element.get()获得当前节点的对应属性
    parsed_header.revMinor =  header.get("revMinor")
    ...
    if header.find("geoReference") is not None:  # 地理参考系，对于可视化而言并不涉及
        # 需要确保 openDrive 含有 proj地理坐标系属性数据，
        ...
        parsed_header._geoReference = proj_string
        

    opendrive._header = parsed_header  # OpenDrive类实例对象的header属性赋值
