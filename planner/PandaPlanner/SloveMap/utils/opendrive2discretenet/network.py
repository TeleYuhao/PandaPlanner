...
from utils.opendrive2discretenet.opendriveparser.elements.opendrive import OpenDrive
...
from utils.opendrive2discretenet.converter import OpenDriveConverter
...
class Network:
    ...
    def load_opendrive(self, opendrive: OpenDrive):
        ...
        self._geoReference = opendrive._header._geoReference
        ...
        for road in opendrive.roads:
            ...
            for lane_section in road.lanes.lane_sections:
                ...
                parametric_lane_groups = OpenDriveConverter.lane_section_to_parametric_lanes(
                    lane_section, reference_border,road.planView.arc
                )
    ...
    def export_discrete_network(
        ...
        for parametric_lane in self._planes:
            if len(parametric_lane.parametric_lanes) == 0:
                continue
            if filter_types is not None and parametric_lane.type not in filter_types:
                continue

            discrete_lane = parametric_lane.to_discretelane(self)
            ...
            discrete_lane.leftlane = self._link_index.get_leftlane(parametric_lane.id_)
            discrete_lane.rightlane = self._link_index.get_rightlane(parametric_lane.id_)

            discrete_network.add_discretelane(discrete_lane)

        return discrete_network

    def getPlanesByID(self,planeID):
...
class LinkIndex:
    ...
    def get_leftlane(self,parametric_lane_id: str) -> str:
        """
        ...
        Returns:
          leftlane id if exist.
        """


        [road_id,section_id,lane_id,width_id] = decode_road_section_lane_width_id(parametric_lane_id)
        # openDrive特性，左边车道 laneid 为 当前车道laneid的绝对值少一，且符号相同,
        ...
        if lane_id < -1:
            ...
            new_lane_id = lane_id - 1 
             ...
             #若当前车道laneid 绝对值为1，说明当前车道为最左边车道
             return ""
        # 编码车道ID
        leftlaneid = encode_road_section_lane_width_id(road_id,section_id,new_lane_id,width_id)


        return leftlaneid

    def get_rightlane(self,parametric_lane_id: str) -> str:
        """
        ...
        Returns:
          rightlane id if exist.
        """
        [road_id,section_id,lane_id,width_id] = decode_road_section_lane_width_id(parametric_lane_id)
        # openDrive特性，左边车道 laneid 为 当前车道laneid的绝对值加一，且符号相同，但需确认此车道是否存在
        ...
        rightlane_id = encode_road_section_lane_width_id(road_id,section_id,new_lane_id,width_id)

        if rightlane_id in self._successors.keys():
            return rightlane_id
        else:
            return ""
