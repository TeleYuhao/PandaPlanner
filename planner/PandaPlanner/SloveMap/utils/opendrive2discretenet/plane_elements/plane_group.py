...
from utils.opendrive2discretenet.discrete_network import DiscreteLane
from pyproj import Proj
from utils.opendrive2discretenet.utils import encode_road_section_lane_width_id
from utils.opendrive2discretenet.utils import decode_road_section_lane_width_id
...
class ParametricLaneGroup:
    ...
    def __init__(
        ...
        self.left_roadMark = left_roadMark
        self.right_roadMark = right_roadMark
    ...
    def to_discretelane(self, network, precision: float = 0.5) -> DiscreteLane:
        ...
        for parametric_lane in self.parametric_lanes:  # 遍历每一宽度区段对应的ParametricLane类对象
            ...
            if parametric_lane.arc is not None:
                curvature = parametric_lane.arc
        ...
        # 添加转换关系，若转换关系解析错误，使用局部坐标
        projection = Proj(network._geoReference)
        left_vertices = np.array(list(map(lambda x: projection(x[0],x[1],inverse=True), left_vertices)))
        right_vertices = np.array(list(map(lambda x: projection(x[0],x[1],inverse=True), right_vertices)))
        ...
        lanelet = DiscreteLane(
            self, left_vertices, center_vertices, right_vertices, self.length, curvature ,self.id_
        )

        lane_type_list = ['none','broken','broken broken','solid','solid solid']
        lane_color_list = ['none','white','yellow']

        if self.left_roadMark is not None:
            lanelet.left_type = lane_type_list.index(self.left_roadMark['type'])
            lanelet.left_color = lane_color_list.index(self.left_roadMark['color'])

        if self.right_roadMark is not None:
            lanelet.right_type = lane_type_list.index(self.right_roadMark['type'])
            lanelet.right_color = lane_color_list.index(self.right_roadMark['color'])
            ## 获取左侧边线
            [road_id,section_id,lane_id,width_id] = decode_road_section_lane_width_id(self.id_)
            if (lane_id < 0 ):
                left_id = lane_id + 1
            else:
                left_id = lane_id - 1
            
            left_lane_id = encode_road_section_lane_width_id(road_id,section_id,left_id,width_id)
            left_road_mark = network.getPlanesByID(left_lane_id).right_roadMark
            lanelet.left_type = lane_type_list.index(left_road_mark['type'])
            lanelet.left_color = lane_color_list.index(left_road_mark['color'])
