...
from utils.opendrive2discretenet.plane_elements.plane import (
    ParametricLane,
    ParametricLaneBorderGroup,
)
from utils.opendrive2discretenet.plane_elements.plane_group import ParametricLaneGroup
from utils.opendrive2discretenet.plane_elements.border import Border
...
class OpenDriveConverter:
    ...
    def lane_section_to_parametric_lanes(
        lane_section, reference_border, planeViewarc
        ...
        for side in ["right", "left", 'center']:
            ...
            lanes = (
                lane_section.rightLanes if side == "right" else lane_section.leftLanes if side == "left" else lane_section.centerLanes
            )
            coeff_factor = -1.0 if side == "right" else  1.0 if side == "left" else 0.0 # 右车道组的车道id从-1开始，左车道组从1开始
            ...
            for lane in lanes:
                ...
                plane_group.right_roadMark = lane_roadMark
                if side == 'center':
                    plane_group.left_roadMark = lane_roadMark
                ...
                for width in lane.widths:
                    parametric_lane = OpenDriveConverter.create_parametric_lane(
                        lane_borders, width, lane, planeViewarc
                    )
    ...
    @staticmethod
    def create_parametric_lane(lane_borders, width, lane, planeViewarc) -> ParametricLane:
